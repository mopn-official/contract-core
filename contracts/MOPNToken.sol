...
contract MOPNToken is ERC20Burnable, Multicall {
    ...
    modifier onlyMOPN() {
        require(msg.sender == address(mopn), "MOPNToken: Only MOPN contract can call this function");
        _;
    }
    ...
    function createCollectionVault(address collectionAddress) public {
        mopn.createCollectionVault(collectionAddress);
    }
    ...
    function totalSupply() public view override returns (uint256) {
        return mopn.MTTotalMinted() + (mopn.calcPerMOPNPointMinted() - mopn.PerMOPNPointMinted()) * mopn.TotalMOPNPoints();
    }
